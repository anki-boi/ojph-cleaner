...
keep the site's own order. On by default (D42), and it waits for the scan on purpose: a card's figure is an
...
| `autoScan` | boolean | `true` | Deep-scan a list page as soon as it loads (W13). On by default (D42): every run is bounded to the recency window, two listings at a time, with hard caps — the `Scan` button does the same thing on demand | `scan.js` |
...
| `goalSalary` | number | `60000` | Monthly PHP goal; cards at or above it brighten (W7, D42) | `salary.js` |
| `goalHourly` | number | `1000` | Hourly PHP goal; posted rates at or above it brighten the card (W13, D13, D42) | `salary-cards.js` |
| `maxAgeDays` | number | `60` | Hide listings posted longer ago than this many days (`0` = off, `7` = last week, `30` = last month). Read **before** every other rule; an unreadable date is never stale (W8, D42) | rule pass |
| `rescueNoSalary` | boolean | `true` | With `noSalary` on: a listing that states no pay but matches a keyword you like (or beats a goal) is shown instead of hidden (W10, D30, D42) | rule pass |
| `rescueNegotiable` | boolean | `true` | With `noSalary` on: a listing that says **Negotiable** or **DOE** and matches a keyword you like is shown with a `⚠ negotiable` tag instead of hidden (0.12.0, D42). Separate from `rescueNoSalary` on purpose — a stated-negotiable listing is a real posting with an unstated figure, not the same thing as a listing that simply never mentions pay | rule pass |
| `sortByPay` | boolean | `true` | Rank the board by the figure on each card when a scan run ends (on by default, D42): highest **monthly** figure first, then posted rates (per hour or per day), then the cards with no figure, in the site's own order (0.14.0, D41). Never converts a month to a rate or back — that would assume a work week (D13) — and the order is re-applied to pages the loader appends later | `pay-sort.js` |
